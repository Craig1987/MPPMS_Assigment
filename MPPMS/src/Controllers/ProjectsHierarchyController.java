...
public class ProjectsHierarchyController implements Observer {
    ...
    public ProjectsHierarchyController(ProjectsHierarchyView view, User currentUser) {
        ...
        this.view.addTreeTransferHandler(new TreeTransferHandler());
    }
    ...
    public void refreshView() {
       ...
       if (projects != null) {
            for (Project project : projects) {
                ...
                if (tasks != null) {
                    for (Task task : tasks) {
                        DefaultMutableTreeNode taskNode = new DefaultMutableTreeNode(task.getTitle());

                        // Get all assets for task node
                        SetOfAssets assets = task.getAssets();
                        if (assets != null) {
                            for (Asset asset : assets) {
                                taskNode.add(new DefaultMutableTreeNode(asset.getDescription()));
                            }
                        }
                ...
                if (coms != null) {
                    for (Component com : coms) {
                        DefaultMutableTreeNode comNode = new DefaultMutableTreeNode(com.getDescription());
                        
                        //Get all assets for component node
                        SetOfAssets assets = com.getAssets();
                        if (assets != null) {
                            for (Asset asset : assets) {
                                comNode.add(new DefaultMutableTreeNode(asset.getDescription()));
                            }
                        }
